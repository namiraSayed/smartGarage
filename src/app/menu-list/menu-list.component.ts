...
export class MenuListComponent {
  ...
  getmenulist() {
    this.spinner.show();
    this.http
      .get( this.serverUrl + `UserProcess/GetAppMenuSearchBy/${this.searchKey}/${this.currentPage}/${this.itemsPerPage}` )
      .subscribe((data: any) => {
        if (data.data.appMenuList == null) {
          this.menudata = [];
          this.notFound = true;
        } else {
          this.menudata = data.data.appMenuList.appMenu;
          this.notFound = false;
        }
  ...
  getPage(event: number) {
    ...
    this.p = event;
    this.http
      .get( this.serverUrl + `UserProcess/GetAppMenuSearchBy/${this.searchKey}/${ this.p - 1 }/${this.itemsPerPage}` )
      .subscribe((data: any) => {
        if (data.data.appMenuList == null) {
          this.menudata = [];
          this.notFound = true;
